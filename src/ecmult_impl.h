...
static size_t secp256k1_strauss_scratch_size_raw(size_t n_points, int as_allocated) {
    ...
    sizes[2] = n_points * ECMULT_TABLE_SIZE(WINDOW_A) * sizeof(secp256k1_gej);
    sizes[3] = n_points * ECMULT_TABLE_SIZE(WINDOW_A) * sizeof(secp256k1_fe);
#ifdef USE_ENDOMORPHISM
    sizes[4] = n_points * 2 * ECMULT_TABLE_SIZE(WINDOW_A) * sizeof(secp256k1_ge);
#else
    sizes[4] = n_points * ECMULT_TABLE_SIZE(WINDOW_A) * sizeof(secp256k1_ge);
#endif
    sizes[5] = n_points * sizeof(struct secp256k1_strauss_point_state);
